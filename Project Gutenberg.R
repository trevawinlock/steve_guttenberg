...
getbooks <- function(x){
  ...
  # Converts encoding for remaining UTF-8 to ASCII
  for (i in 1:nrow(bookinfo)) {
    if(Encoding(bookinfo[i,2]) == "UTF-8"){
      
      bookinfo[i,2] <- iconv(bookinfo[i,2], "UTF-8", "ASCII")
      
      books[i,2] <- iconv(books[i,2], "UTF-8", "ASCII")
    } else {
      next
    }
  }
...
# Function to generate a list of reading levels
get_reading_level <- function(df) {
  bookinfo1 <- NULL
  for (i in 1:nrow(df)) {
    bookinfo1[[i]] <- readability(df[i,2], NULL)
  }
  gunning_fog <- map(bookinfo1, "Gunning_Fog_Index")
}


system.time(getbooks(1000))

# Get reading levels and add to bookinfo dataframe
system.time(bookinfo$reading_level <- get_reading_level(bookinfo))

# Remove any text that is now NA due to Encoding conversion
bookinfo <- na.omit(bookinfo)
